...
mod tests {
    ...
    fn test_store_and_retrieve_db_key() {
        ...
        assert_eq!(original_key, retrieved_key, "Retrieved key should match stored key");
    ...
    fn test_get_or_create_creates_new_key() {
        ...
        assert_eq!(key1, key2, "get_or_create should return same key on subsequent calls");
    ...
    fn test_retrieve_nonexistent_key_returns_error() {
        ...
        assert!(result.is_err(), "Retrieving non-existent key should return error");
    }
    ...
    fn test_key_file_permissions_on_unix() {
        #[cfg(unix)]
        {
            ...
            assert_eq!(permissions.mode() & 0o777, 0o600, "Key file should have 0600 permissions");
