...
pub async fn rotate_signed_pre_key<S: SignalStorageContainer>(
    ...
    let current_max_id =
        storage.signed_pre_key_store().get_max_signed_pre_key_id().await?.unwrap_or(0);
...
pub async fn signed_pre_key_needs_rotation<S: SignalStorageContainer>(
    ...
    let max_id = storage.signed_pre_key_store().get_max_signed_pre_key_id().await?;
    ...
    let current_key =
        storage.signed_pre_key_store().get_signed_pre_key(SignedPreKeyId::from(id)).await?;
...
pub async fn cleanup_expired_signed_pre_keys<S: SignalStorageContainer>(
    ...
    let expired_ids = storage.signed_pre_key_store().get_signed_pre_keys_older_than(cutoff).await?;
    ...
    for id in expired_ids {
        if storage.signed_pre_key_store().signed_pre_key_count().await > 1 {
            storage.signed_pre_key_store().delete_signed_pre_key(id).await?;
        }
    }
...
pub async fn replenish_pre_keys<S: SignalStorageContainer>(
    ...
    let next_id = storage.pre_key_store().get_max_pre_key_id().await?.map(|id| id + 1).unwrap_or(1);
...
pub async fn rotate_kyber_pre_key<S: SignalStorageContainer>(
    ...
    let current_max_id =
        storage.kyber_pre_key_store().get_max_kyber_pre_key_id().await?.unwrap_or(0);
...
pub async fn kyber_pre_key_needs_rotation<S: SignalStorageContainer>(
    ...
    let max_id = storage.kyber_pre_key_store().get_max_kyber_pre_key_id().await?;
    ...
    let current_key =
        storage.kyber_pre_key_store().get_kyber_pre_key(KyberPreKeyId::from(id)).await?;
...
pub async fn cleanup_expired_kyber_pre_keys<S: SignalStorageContainer>(
    ...
    let expired_ids = storage.kyber_pre_key_store().get_kyber_pre_keys_older_than(cutoff).await?;
    ...
    for id in expired_ids {
        if storage.kyber_pre_key_store().kyber_pre_key_count().await > 1 {
            storage.kyber_pre_key_store().delete_kyber_pre_key(id).await?;
        }
    }
...
mod tests {
    ...
    async fn test_rotate_signed_pre_key_generates_new_key() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        storage.signed_pre_key_store().save_signed_pre_key(initial_key.id()?, &initial_key).await?;
        ...
        assert_eq!(storage.signed_pre_key_store().signed_pre_key_count().await, 2);
    ...
    async fn test_signed_pre_key_needs_rotation_when_old() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        storage.signed_pre_key_store().save_signed_pre_key(old_key.id()?, &old_key).await?;

        let needs_rotation = signed_pre_key_needs_rotation(&mut storage).await?;
        assert!(needs_rotation, "Key older than rotation interval should need rotation");
    ...
    async fn test_signed_pre_key_does_not_need_rotation_when_fresh(
        ...
        storage.signed_pre_key_store().save_signed_pre_key(fresh_key.id()?, &fresh_key).await?;
    ...
    async fn test_cleanup_expired_signed_pre_keys() -> Result<(), Box<dyn std::error::Error>> {
        ...
        storage.signed_pre_key_store().save_signed_pre_key(expired_key.id()?, &expired_key).await?;
    ...
    async fn test_consume_pre_key_removes_key() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 1..=51 {
            ...
            storage.pre_key_store().save_pre_key(i.into(), &record).await?;
        }
    ...
    async fn test_replenish_pre_keys_when_low() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(storage.pre_key_store().pre_key_count().await, REPLENISH_COUNT as usize);
    ...
    async fn test_kyber_pre_key_needs_rotation_when_old() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        assert!(needs_rotation, "Kyber key older than rotation interval should need rotation");
