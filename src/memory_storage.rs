...
impl MemoryPreKeyStore {
    pub fn new() -> Self {
        Self { pre_keys: Arc::new(Mutex::new(HashMap::new())) }
    }
}
...
impl MemorySignedPreKeyStore {
    pub fn new() -> Self {
        Self { signed_pre_keys: Arc::new(Mutex::new(HashMap::new())) }
    }
}
...
impl MemoryKyberPreKeyStore {
    pub fn new() -> Self {
        Self { kyber_pre_keys: Arc::new(Mutex::new(HashMap::new())) }
    }
}
...
mod tests {
    ...
    async fn test_key_generation_storage_integration() -> Result<(), Box<dyn std::error::Error>> {
        ...
        storage
            ...
            .await?;
        let retrieved_signed_key =
            storage.signed_pre_key_store.get_signed_pre_key(signed_pre_key.id()?).await?;
