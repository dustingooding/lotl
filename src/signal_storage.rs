...
impl SqliteStorage {
    /// Creates a new SQLite storage instance using a shared connection
...
impl SignalStorageContainer for SqliteStorage {
    ...
    fn initialize(&mut self) -> std::result::Result<(), Box<dyn std::error::Error>> {
        self.initialize_schema().map_err(|e| Box::new(e) as Box<dyn std::error::Error>)
    }
...
impl SessionStore for SqliteSessionStore {
    async fn load_session(
        ...
        let result =
            stmt.query_row([address.name(), &u32::from(address.device_id()).to_string()], |row| {
        ...
        match result {
            ...
            Err(e) => {
                Err(SignalProtocolError::InvalidState("storage", format!("Database error: {}", e)))
            }
        }
    }
...
impl ExtendedSessionStore for SqliteSessionStore {
    ...
    async fn clear_all_sessions(&mut self) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
...
impl IdentityKeyStore for SqliteIdentityStore {
    async fn get_identity_key_pair(
        ...
        let mut stmt =
            conn.prepare("SELECT private_key FROM local_identity WHERE id = 1").map_err(|e| {
                SignalProtocolError::InvalidState(
                    "storage",
                    format!("Failed to prepare statement: {}", e),
                )
...
impl ExtendedIdentityStore for SqliteIdentityStore {
    ...
    async fn get_peer_identity(
        &self,
        address: &ProtocolAddress,
    ) -> std::result::Result<Option<IdentityKey>, Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT public_key FROM identity_keys WHERE address = ? AND device_id = ?")?;
...
impl SqlitePreKeyStore {
    /// Creates a new pre-key store
    pub fn new(connection: Arc<Mutex<Connection>>) -> Self {
        Self { connection }
    }

    /// Creates the pre_keys table
...
impl PreKeyStore for SqlitePreKeyStore {
    async fn get_pre_key(
        ...
        let conn = self.connection.lock().unwrap();
        let mut stmt = conn.prepare("SELECT key_data FROM pre_keys WHERE id = ?").map_err(|e| {
            SignalProtocolError::InvalidState(
                "storage",
                format!("Failed to prepare statement: {}", e),
            )
    ...
    async fn remove_pre_key(
        ...
        let conn = self.connection.lock().unwrap();
        conn.execute("DELETE FROM pre_keys WHERE id = ?", rusqlite::params![u32::from(prekey_id)])
            .map_err(|e| {
                SignalProtocolError::InvalidState(
                    "storage",
                    format!("Failed to remove pre key: {}", e),
                )
            })?;
        Ok(())
    }
...
impl ExtendedPreKeyStore for SqlitePreKeyStore {
    ...
    async fn clear_all_pre_keys(&mut self) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
        conn.execute("DELETE FROM pre_keys", [])?;
        Ok(())
    }

    async fn get_max_pre_key_id(
        &self,
...
impl SqliteSignedPreKeyStore {
    /// Creates a new signed pre-key store
    pub fn new(connection: Arc<Mutex<Connection>>) -> Self {
        Self { connection }
    }

    /// Creates the signed_pre_keys table
    pub fn create_tables(conn: &Connection) -> Result<()> {
        conn.execute(
...
impl SignedPreKeyStore for SqliteSignedPreKeyStore {
    async fn get_signed_pre_key(
        ...
        let conn = self.connection.lock().unwrap();
        let mut stmt =
            conn.prepare("SELECT key_data FROM signed_pre_keys WHERE id = ?").map_err(|e| {
                SignalProtocolError::InvalidState(
                    "storage",
                    format!("Failed to prepare statement: {}", e),
                )
    ...
    async fn save_signed_pre_key(
        ...
        let expires_at = record_timestamp_secs + (30 * 24 * 60 * 60);
...
impl ExtendedSignedPreKeyStore for SqliteSignedPreKeyStore {
    ...
    async fn clear_all_signed_pre_keys(
        &mut self,
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
    ...
    async fn get_max_signed_pre_key_id(
        &self,
    ) -> std::result::Result<Option<u32>, Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
    ...
    ) -> std::result::Result<Vec<SignedPreKeyId>, Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
...
impl SqliteKyberPreKeyStore {
    /// Creates a new Kyber pre-key store
    pub fn new(connection: Arc<Mutex<Connection>>) -> Self {
        Self { connection }
    }

    /// Creates the kyber_pre_keys table
    pub fn create_tables(conn: &Connection) -> Result<()> {
        conn.execute(
...
impl KyberPreKeyStore for SqliteKyberPreKeyStore {
    async fn get_kyber_pre_key(
        ...
        let conn = self.connection.lock().unwrap();
        let mut stmt =
            conn.prepare("SELECT key_data FROM kyber_pre_keys WHERE id = ?").map_err(|e| {
                SignalProtocolError::InvalidState(
                    "storage",
                    format!("Failed to prepare statement: {}", e),
                )
...
impl ExtendedKyberPreKeyStore for SqliteKyberPreKeyStore {
    ...
    async fn clear_all_kyber_pre_keys(
        &mut self,
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
    ...
    async fn get_max_kyber_pre_key_id(
        &self,
    ) -> std::result::Result<Option<u32>, Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
    ...
    async fn get_kyber_pre_keys_older_than(
        &self,
        timestamp_millis: u64,
    ) -> std::result::Result<Vec<KyberPreKeyId>, Box<dyn std::error::Error>> {
        let conn = self.connection.lock().unwrap();
        let timestamp_secs = (timestamp_millis / 1000) as i64;
...
mod tests {
    use super::*;
    use libsignal_protocol::{kem, Timestamp};

    #[tokio::test]
    async fn test_sqlite_storage_creation() -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
        let storage = SqliteStorage::new(connection).await?;
        assert_eq!(storage.storage_type(), "sqlite");
        Ok(())
    }

    #[tokio::test]
    async fn test_sqlite_storage_schema_initialization(
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
    ...
    #[tokio::test]
    async fn test_sqlite_session_store_count_empty(
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
    ...
    #[tokio::test]
    async fn test_sqlite_identity_store_count_empty(
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
    ...
    #[tokio::test]
    async fn test_sqlite_pre_key_store_count_empty(
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
    ...
    #[tokio::test]
    async fn test_sqlite_signed_pre_key_store_count_empty(
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
    ...
    #[tokio::test]
    async fn test_sqlite_kyber_pre_key_store_count_empty(
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
    ...
    #[tokio::test]
    async fn test_sqlite_storage_container() -> std::result::Result<(), Box<dyn std::error::Error>>
    {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
        let mut storage = SqliteStorage::new(connection).await?;
        storage.initialize()?;
    ...
    #[tokio::test]
    async fn test_identity_key_store_trait() -> std::result::Result<(), Box<dyn std::error::Error>>
    {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
        let mut storage = SqliteStorage::new(connection).await?;
        storage.initialize_schema()?;
    ...
    #[tokio::test]
    async fn test_pre_key_store_trait() -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
        SqlitePreKeyStore::create_tables(&connection.lock().unwrap())?;
    ...
    #[tokio::test]
    async fn test_signed_pre_key_store_trait() -> std::result::Result<(), Box<dyn std::error::Error>>
    {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
        SqliteSignedPreKeyStore::create_tables(&connection.lock().unwrap())?;
    ...
    #[tokio::test]
    async fn test_kyber_pre_key_store_trait() -> std::result::Result<(), Box<dyn std::error::Error>>
    {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
        SqliteKyberPreKeyStore::create_tables(&connection.lock().unwrap())?;
    ...
    #[tokio::test]
    async fn test_sqlite_storage_close() -> std::result::Result<(), Box<dyn std::error::Error>> {
        let connection = Arc::new(Mutex::new(Connection::open(":memory:")?));
        let mut storage = SqliteStorage::new(connection).await?;
        storage.initialize_schema()?;
